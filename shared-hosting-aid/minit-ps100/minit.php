...
class Minit {
	...
	private $async_style_queue = array();
	...
	private function __construct() {
        ...
        add_action( 'wp_head', array( $this, 'async_init_styles' ) , 8 );
        add_action( 'wp_print_footer_scripts', array( $this, 'async_print_styles' ) , 19 );
...
//error_log('minit.NOW= '. $extension . serialize($todo));
...
//if ($extension=='js') error_log('O='.count($minit_todo));
...
//if ($extension=='js') error_log($extension . ': ' . implode( '| ', $ver) . serialize(did_action('get_footer')). "\n" );
...
//if ($extension=='js') error_log('FE'.$script);
	...
	function minit_enqueue_files( &$object, $status ) {
		...
		switch ( $extension ) {
			...
			case 'js':
				...
				foreach ( $done as $script ) {
//error_log('s:'. $script . ': ' . $object->get_data( $script, 'data' ) );
...
<?php
	...
	public function async_init_styles() {
//error_log('NOW=init-sytles');
		...
		$base_url = site_url();
		$minit_exclude = (array) apply_filters( 'minit-exclude-css', array() );

		foreach ( $wp_styles->queue as $handle ) {
			// Skip asyncing explicitly excluded style handles
			if ( in_array( $handle, $minit_exclude ) ) {
				continue;
			}

			// Add this style to our async queue
			$this->async_style_queue[] = $handle;

			// Remove this style from being printed the regular way
	...
	public function async_print_styles() {

		global $wp_styles;

		if ( empty( $this->async_style_queue ) )
			return;

		?>
<!-- Asynchronous styles by Minit -->
<script id="minit-async-styles" type="text/javascript">
(function () {
	var css, inlineId, inlineCss,
	    headE = document.getElementsByTagName('head')[0],
		add = function( url, media, id ) {
			css = document.createElement('link');
			css.rel = 'stylesheet';
			css.media = media;
			css.href = url;
			css.id = id;
			inlineId = id.replace('-async-style', '-inline-css');
			inlineCss = document.getElementById(inlineId);
			if (inlineCss === null) {
			    headE.appendChild(css);
			} else {
			    inlineCss.parentNode.insertBefore(css, inlineCss);
			}
		};
<?php
	foreach ( $this->async_style_queue as $handle ) {
...
add_filter( 'style_loader_tag', 'minit_hide_styles' );
function minit_hide_styles( $tag ) {
...
/*
// Keep jQuery in the head for fiddling.
add_filter( 'minit-exclude-js', 'minit_js_excludes' );
function minit_js_excludes( $excludes ) {
